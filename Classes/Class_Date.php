...
class Class_Date extends DateTime
{
	protected $_year;
	protected $_month;
	protected $_day;
	...
	public function setTime($hours, $minutes, $seconds = 0)
	{
		...
		{
			throw new Exception("setTime() expect two or three numbers separated by commas in the order: hours, minutes, seconds");
		...
		if ($outOfRange)
		{
			throw new Exception("Invalid time.", 1);
	...
	public function setDate($year, $month, $day)
	{
		...
		{
			throw new Exception('setDate() expects three numbers sparated by commas in the order: year, month, day');
	...
	public function setMDY($USDate)
	{
		...
		if (!is_array($dateParts) || cont($dateParts) != 3)
		{
			throw new Exception('setDMY() expects a date as "MM/DD/YYYY".');
	...
	public function subMonths($numMonths)
	{
		...
		{
			throw new Exception('addMonths() ecpects an integer. ');
	...
	public function addYears($numYears)
	{
		...
		{
			throw new Exception('addYear() expects a positive integer.');
	...
	static public function dateDiff(Class_Date $startDate, Class_Date $endDate)
	{
